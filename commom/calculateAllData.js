//计算币种汇率转换
function transformCurrency(type, number, rate) {
    if (number) {
 ...
 * @param info sku商品信息
 */
function calculateGrossMargin(type, params, info) {
    //将数据全部转为数字
...
//换算成人民币和美元
function calculateCurrency(list) {
    const cnyList = [];
    const usdList = [];
    /**
     * 将数据区分成人民币列表跟美元列表
    ...
    list.forEach((element) => {
        const exchangeRate = Number(element.exchangeRate);
        //将美元转化为人民币
        const cnyObj = {};
        usdKey.forEach((key) => {
...
//计算总额
function calculateAllData(list) {
    const cnyDataSource = calculateCurrency(list).cnyList
    const usdDataSource = calculateCurrency(list).usdList
    const cnyData = {
        ...
        procurementCostTotal: (Number(sum(cnyDataSource, 'businessVolume')) / (Number(sum(cnyDataSource, 'orderQuantity', true)) ? Number(sum(cnyDataSource, 'orderQuantity', true)) : 1)).toFixed(2),
        serviceChargeTotal: sum(cnyDataSource, 'serviceCharge'),
        estimatedProfitTotal: sum(cnyDataSource, 'estimatedProfit'),
        roiTotal: Number(sum(cnyDataSource, 'adRate')) ? (Number(sum(cnyDataSource, 'businessVolume')) / Number(sum(cnyDataSource, 'adRate'))).toFixed(2) : '0',
        homogeneousAdrateTotal: (Number(sum(cnyDataSource, 'adRate')) / (Number(sum(cnyDataSource, 'orderQuantity', true)) ? Number(sum(cnyDataSource, 'orderQuantity', true)) : 1)).toFixed(2),  //单均广告费
        grossProfitRatioTotal:
            (
                (Number(sum(cnyDataSource, 'estimatedProfit')) /
                    (Number(sum(cnyDataSource, 'businessVolume')) ? Number(sum(cnyDataSource, 'businessVolume')) : 1)) *
                100
            ).toFixed(2) + '%',
    };
    const usdData = {
        ...
        procurementCostTotal: (Number(sum(usdDataSource, 'businessVolume')) / (Number(sum(usdDataSource, 'orderQuantity', true)) ? Number(sum(usdDataSource, 'orderQuantity', true)) : 1)).toFixed(2),
        serviceChargeTotal: sum(usdDataSource, 'serviceCharge'),
        estimatedProfitTotal: sum(usdDataSource, 'estimatedProfit'),
        roiTotal: Number(sum(usdDataSource, 'adRate')) ? (Number(sum(usdDataSource, 'businessVolume')) / Number(sum(usdDataSource, 'adRate'))).toFixed(2) : '0',
        homogeneousAdrateTotal: (Number(sum(usdDataSource, 'adRate')) / (Number(sum(usdDataSource, 'orderQuantity', true)) ? Number(sum(usdDataSource, 'orderQuantity', true)) : 1)).toFixed(2),  //单均广告费
        grossProfitRatioTotal:
            (
                (Number(sum(usdDataSource, 'estimatedProfit')) /
                    (Number(sum(usdDataSource, 'businessVolume')) ? Number(sum(usdDataSource, 'businessVolume')) : 1)) *
                100
            ).toFixed(2) + '%',
    };
    return {
...
module.exports = calculateAllData;